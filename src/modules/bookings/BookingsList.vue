<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
      <div>
        <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight leading-none">{{ t('bookings.title') }}</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-2 text-sm font-medium">{{ t('bookings.subtitle') }}</p>
      </div>
      <router-link
        to="/vehicles/map"
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-24 bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
      ...
      <p class="text-gray-400 text-xs font-black uppercase tracking-widest">{{ t('bookings.syncing') }}</p>
    </div>
    ...
    <div v-else-if="bookings.length === 0" class="flex flex-col items-center justify-center py-20 bg-white dark:bg-gray-900 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm text-center px-6">
      ...
      <h3 class="text-xl font-black text-gray-900 dark:text-white mb-2 uppercase tracking-tight">{{ t('bookings.empty_title') }}</h3>
      <p class="text-gray-500 text-sm max-w-xs mb-8 font-medium">{{ t('bookings.empty_subtitle') }}</p>
      <router-link
        to="/vehicles/map"
    ...
    <div v-else class="space-y-10">
      ...
      <section v-if="activeBookings.length > 0" class="space-y-6">
        <h2 class="text-[10px] font-black text-brand-primary-500 uppercase tracking-[0.3em] ml-4">{{ t('bookings.in_progress') }}</h2>
          ...
          >
            ...
            <div class="flex items-start gap-6 mb-8 relative z-10">
              <div class="size-20 rounded-3xl bg-gray-50 dark:bg-gray-800 overflow-hidden group-hover:scale-105 transition-transform duration-500 shadow-inner">
                <img :src="getVehicleImage(booking.vehicle?.brand, booking.vehicle?.model)" class="size-full object-cover" :alt="t('bookings.vehicle_alt')" />
              </div>
              <div class="text-left flex-1 min-w-0">
                ...
                <div v-if="booking.status === 'pending' && (booking.remaining_seconds ?? 0) > 0" class="mt-4 flex items-center gap-2 text-amber-600 dark:text-amber-400">
                  ...
                  <span class="text-xs font-black uppercase tracking-widest">{{ t('bookings.expires_in') }}: {{ formatRemainingTime(booking.remaining_seconds) }}</span>
                </div>
                ...
                <div v-if="booking.status === 'active'" class="mt-4 flex items-center gap-2 text-green-600 dark:text-green-400">
                  ...
                  <span class="text-xs font-black uppercase tracking-widest">{{ t('bookings.trip_active') }}</span>
                </div>
              </div>
            </div>
      ...
      <section v-if="pastBookings.length > 0" class="space-y-6">
        <h2 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] ml-4">{{ t('bookings.history_title') }}</h2>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left">
              <thead class="bg-gray-50/50 dark:bg-gray-950/50 border-b border-gray-100 dark:border-gray-800 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                <tr>
                  <th class="px-8 py-5">{{ t('bookings.table_vehicle') }}</th>
                  <th class="px-8 py-5">{{ t('bookings.table_date') }}</th>
                  <th class="px-8 py-5 text-right">{{ t('bookings.table_duration_cost') }}</th>
                  <th class="px-8 py-5 text-center">{{ t('bookings.table_status') }}</th>
                </tr>
              </thead>
    ...
    <ConfirmDialog 
      ...
      :title="t('bookings.cancel_dialog_title')"
      :message="t('bookings.cancel_dialog_message')"
...
const handleActivate = async (id: number) => {
  try {
    ...
    showToast(t('bookings.activate_success'), 'success')
    // Redirigir directamente al panel de control activo
    router.push('/active-vehicle')
  } catch (e: any) {
    showToast(e.response?.data?.message || t('bookings.activate_error'), 'error')
  }
}
...
const confirmCancel = async () => {
  ...
  try {
    ...
    showToast(t('bookings.cancel_success'), 'success')
    getBookings()
  } catch (e: any) {
    showToast(e.response?.data?.message || t('bookings.cancel_error'), 'error')
  } finally {
