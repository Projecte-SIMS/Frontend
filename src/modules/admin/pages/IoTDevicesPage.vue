          ...
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
              <tr v-for="device in allDevices" :key="device.id" class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                ...
                <td class="px-6 py-4 text-center"><span :class="device.online ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-50 text-slate-500 border-slate-200'" class="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border">{{ device.online ? 'En línea' : 'Sin conexión' }}</span></td>
