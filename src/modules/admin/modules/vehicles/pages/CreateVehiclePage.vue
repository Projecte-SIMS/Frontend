<template>
  <div class="space-y-8 animate-fade-in">
    <PageHeading title="Crear Vehículo" description="Añade un nuevo vehículo a la flota.">
      <template #actions>
        <router-link
          to="/admin/vehicles"
      ...
      <form @submit.prevent="handleSubmit">
        ...
        >
          ...
          <button
            type="submit"
            :disabled="loading"
            class="flex items-center space-x-2 bg-brand-primary-600 text-white hover:bg-brand-primary-700 disabled:opacity-50 disabled:cursor-not-allowed text-[10px] font-black uppercase tracking-widest px-4 py-2.5 rounded-xl"
          >
            <svg
              v-if="loading"
            ...
            <span>{{ loading ? 'Guardando...' : 'Crear Vehículo' }}</span>
          </button>
        </div>
      </form>
    </div>
